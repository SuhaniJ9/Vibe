...
const AddCompetition = ({close}) => {
    ...
    const clubForm = useFormik({
        ...
        onSubmit: (values) => {
            ...
            fetch('http://localhost:5000/clubs/add', {
                method: 'POST',
    ...
    .then((res) => res.json()) // Assuming your backend returns a JSON response
    .then((data) => {
        if (data.status === 200) {
            console.log("file uploaded");
            clubForm.setFieldValue('image', data.fileName);  // Assuming backend returns file name in response
        }
    })
    .catch((err) => {
